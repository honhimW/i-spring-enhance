...
public class JsonWhereClause {
    ...
    public static <T> Specification<T> spec(String fieldName, List<IPageRequest.ConditionColumn> attrs) {
        Validate.validState(StringUtils.isNotBlank(fieldName), "fieldName can not be blank");
        return (root, query, cb) -> {
            List<Predicate> predicates = new ArrayList<>();
            attrs.forEach(conditionColumn -> {
                String name = conditionColumn.getName();
                Object value = conditionColumn.getValue();
                IPageRequest.MatchingType type = conditionColumn.getType();
                String key = StringUtils.removeStart(name, fieldName + ".");
                Validate.validState(StringUtils.isNotBlank(key), "key can not be blank");
