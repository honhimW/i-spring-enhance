...
public class HashUtils {
    ...
    private static final String HEX_DIGITS_LOWER_CAST = "0123456789abcdef";

    private static final String HEX_DIGITS_UPPER_CAST = "0123456789ABCDEFG";
    ...
    private char[] charset = HEX_DIGITS_LOWER_CAST.toCharArray();
    ...
    public static String toHexString(byte[] digest) {
        return toHexString(digest, HEX_DIGITS_LOWER_CAST.toCharArray());
    }
    ...
    public static String md5(byte[] bytes) {
        return md5().update(bytes).toHexString();
    }

    public static String sha1(byte[] bytes) {
        return sha1().update(bytes).toHexString();
    }

    public static String sha256(byte[] bytes) {
        return sha256().update(bytes).toHexString();
    }

    public static String sha512(byte[] bytes) {
        return sha512().update(bytes).toHexString();
    }
