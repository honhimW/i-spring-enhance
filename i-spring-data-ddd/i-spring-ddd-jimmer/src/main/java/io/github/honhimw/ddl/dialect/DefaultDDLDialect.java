    ...
    public String columnType(int jdbcType, Long length, Integer precision, Integer scale) {
        ...
        return switch (jdbcType) {
            ...
            case FLOAT ->
                ...
                DDLUtils.replace("float($p)", null, precision, null);
            case REAL ->
            ...
            case NUMERIC -> DDLUtils.replace("numeric($p,$s)", null, precision, scale);
            case DECIMAL -> DDLUtils.replace("decimal($p,$s)", null, precision, scale);
            case DATE -> "date";
            case TIME -> DDLUtils.replace("time($p)", null, precision, null);
            case TIME_WITH_TIMEZONE ->
                ...
                DDLUtils.replace("time($p) with time zone", null, precision, null);
            case TIMESTAMP -> DDLUtils.replace("timestamp($p)", null, precision, null);
            case TIMESTAMP_WITH_TIMEZONE -> DDLUtils.replace("timestamp($p) with time zone", null, precision, null);
            case CHAR -> DDLUtils.replace("char($l)", length, null, null);
            case VARCHAR -> DDLUtils.replace("varchar($l)", length, null, null);
            case CLOB -> "clob";
            case NCHAR -> DDLUtils.replace("nchar($l)", length, null, null);
            case NVARCHAR -> DDLUtils.replace("nvarchar($l)", length, null, null);
            case NCLOB -> "nclob";
            case BINARY -> DDLUtils.replace("binary($l)", length, null, null);
            case VARBINARY -> DDLUtils.replace("varbinary($l)", length, null, null);
            case BLOB -> "blob";
            default -> throw new IllegalArgumentException("unknown type: " + jdbcType);
        };
    }
    ...
    public int resolveJdbcType(Class<?> sqlType, EnumType.Strategy strategy) {
        if (sqlType.isEnum()) {
            return switch (strategy) {
                case NAME -> Types.VARCHAR;
                case ORDINAL -> Types.SMALLINT;
            };
        }
