...
public class AclSubQuerySpecification<T, S extends T> implements Specification<S> {
    ...
    private final IPageRequest.MatchingType matchingType;

    private final Map<String, Object> attributes;

    public AclSubQuerySpecification(JpaEntityInformation<T, ?> hostEI, EntityType<?> subEntityType, String hostParamName, String rtProperty, String property, String value, IPageRequest.MatchingType matchingType, Map<String, Object> attributes) {
        this.hostEI = hostEI;
    ...
    public Predicate toPredicate(@Nonnull Root<S> root, @Nullable CriteriaQuery<?> query, @Nonnull CriteriaBuilder criteriaBuilder) {
        ...
        if (matchingType == IPageRequest.MatchingType.IN
            && StringUtils.startsWith(value, "[")
            && StringUtils.endsWith(value, "]")) {
            try {
                finalValue = JsonUtils.getObjectMapper().readerFor(List.class).readValue(value);
        ...
        Predicate predicate = new IExampleSpecification<>(IPageRequest.ConditionColumn.of(property, finalValue, matchingType)).toPredicate(from, query, criteriaBuilder);
